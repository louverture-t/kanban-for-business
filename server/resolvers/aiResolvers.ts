...
// --- Rate limiting: in-memory per-user AI call bucket ---
...
function assertNotRateLimited(userId: string): void {
    const now = Date.now();
    const existing = aiBuckets.get(userId);

  if (!existing || existing.resetAt <= now) {
        aiBuckets.set(userId, { count: 1, resetAt: now + RATE_LIMIT_WINDOW_MS });
        return;
  }

  if (existing.count >= MAX_AI_CALLS) {
        throw new ValidationError('Rate limit exceeded: too many AI requests');
  }

  existing.count += 1;
}

/** Exported for test teardown only — clears the in-memory rate-limit state. */
export function __resetAiRateLimiterForTests(): void {
    aiBuckets.clear();
}
...
export const aiResolvers = {
    Mutation: {
          aiDecompose: async (
                  ...
                  assertNotRateLimited(context.user!.id);
